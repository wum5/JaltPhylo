############################################################
## modules to treat post-aligned coding sequences.
...
class OrfAln:
	...
	def site_depth(self, seqname):
		...
		for record in self.fileFASTA:
			...
			for x in xrange(len(codon_cov)):
				if record.seq[x*3] in nucleotides and record.seq[x*3+1] in nucleotides and record.seq[x*3+2] in nucleotides and record.id != seqname:
					codon_cov[x] += 1
				else:
					pass
			num += 1 
	...
	def seq_filter(self, seqname, length_cutoff, depth_cutoff):
		...
		for key in seq_dict:
			currLen = seq_dict[key].count('A')+seq_dict[key].count('C')+seq_dict[key].count('G')+seq_dict[key].count('T')
...
if __name__ == "__main__":
	...
	parser.add_argument('-s', '--sequence_delete', default="Capana")
	parser.add_argument('-l', '--length_cutoff', default=200)
	parser.add_argument('-d', '--depth_cutoff', default=15)
	args = parser.parse_args()
	
	inDIR = args.input_folder+"/"
	outDIR = args.output_folder+"/"
	seqname = args.sequence_delete
	lencut = args.length_cutoff
	depcut = args.depth_cutoff
