...
REPEATS = 200 #Number of jackknife repeats
ICC = False #whether or not to use the raxml version compiled with intel compilor
...
if __name__ == "__main__":
	...
	for i in range(REPEATS):
		...
		else: raxml_call = "raxmlHPC-PTHREADS-SSE3"
		cmd = raxml_call+" -F -T "+num_core+" -E "+exc_file+" -m "+MODEL+" -p 6666 -q "+model_file
		cmd += " -s "+aln+" -n "+aln.replace("phy",exc_file)
		print cmd
		os.system(cmd) #write new matrix
		cmd = raxml_call+" -F -T "+num_core+" -m "+MODEL+" -p 6666 -q "+model_file+"."+exc_file
